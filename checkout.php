...
DEL;
					...
					<?php
						if (isset($_POST['deliver_to_saved'])) {
							...
							$query4 = "INSERT INTO `orders`(`c_id`, `a_id`, `o_datetime`, `o_type`, `o_status`) VALUES ({$c_id}, {$a_id}, now(), 'Deliver', 'Placed')";
							...
							$item = (count($product)-7)/5;
					       	...
					       	for($i=1; $i<=$item; $i++){
								...
								$query_result5 = query($query5);
								$total = $total + $product['quantity_'.$i]*$product['amount_'.$i];
							}
							if($total<500){
								$total+=40;
							}
					...
					?>
						...
						if (isset($_POST['pickup'])) {
							...
							$query4 = "INSERT INTO `orders`(`c_id`, `a_id`, `o_datetime`, `o_type`, `o_status`) VALUES ({$c_id}, {$a_id}, now(), 'Pickup', 'Placed')";
							$query_result4 = query($query4);
