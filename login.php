...
//check user exist
				...
				if (isset($_POST['login'])) {
					...
					$enc_password = md5($password);
//check user exist
					$query = "SELECT c_name, c_email, c_id, password from `customer` where c_email = '{$email}' AND password = '{$enc_password}'";
					$query_result = query($query);
					$query_value = fetch_array($query_result);
					if(mysqli_num_rows($query_result) == 1){
						$_SESSION['c_id'] = $query_value['c_id'];
						$_SESSION['c_name'] = $query_value['c_name'];
						$_SESSION['password'] = $query_value['password'];
						// echo $_SESSION['c_id'];
						echo "<script>window.location.href='index.php';</script>";
						die();
						...
						<script type="text/javascript">
							...
							document.getElementById('un_reg').innerHTML = "User id or password is incorrect";
						</script>
						<?php
					}
				}
